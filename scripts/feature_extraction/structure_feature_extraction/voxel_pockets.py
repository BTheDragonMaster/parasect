...
from parasect.scripts.math.shapes import Cube, Sphere, Vector3D
from parasect.scripts.math.distance_calculations import intersection_cube_sphere
from parasect.scripts.parsers.pdb import atoms_from_pdb
from parasect.scripts.parsers.voxel import Voxel, VOXEL_TYPE_TO_ABBREVIATION
from parasect.scripts.data_processing.structure_processing.voxel_adjacency_matrix import (
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Input directory containing pdb files of A-domains.",
    )
                ...
                if os.path.isfile(pdb_path) and pdb_path.endswith(".pdb"):
                    ...
                    if not wrote_header:
                        ...
                        for i, voxel in enumerate(voxel_grid):
                            features.write(f"{voxel.__repr__()}|is_empty\t")
                            features.write(f"{voxel.cube.midpoint.__repr__()}|is_pocket\t")
                            features.write(f"{voxel.cube.midpoint.__repr__()}|nr_aromatic\t")
                            features.write(f"{voxel.cube.midpoint.__repr__()}|nr_pos_charge\t")
                            features.write(f"{voxel.cube.midpoint.__repr__()}|nr_neg_charge\t")
                            features.write(f"{voxel.cube.midpoint.__repr__()}|nr_pos_polar\t")
                            features.write(f"{voxel.cube.midpoint.__repr__()}|nr_neg_polar\t")
                            features.write(f"{voxel.cube.midpoint.__repr__()}|nr_sulfur\t")
                            features.write(f"{voxel.cube.midpoint.__repr__()}|nr_hydrophobic\t")
                            features.write(f"{voxel.cube.midpoint.__repr__()}|nr_metal\t")
                            features.write(f"{voxel.cube.midpoint.__repr__()}|nr_phosphorus")
...
def pdb_to_voxel_representation(
    pdb_file: str,
    voxel_width: float,
