...
SIGNATURES = os.path.join(
    ...
    "active_site_34_hmm.fasta",
)
DATASET = os.path.join(os.path.dirname(parasect.data.__file__), "parasect_dataset.txt")
DOMAIN_TO_SPEC = parse_specificities(DATASET)
DOMAIN_LIST = os.path.join(os.path.dirname(parasect.data.__file__), "domain_list_hmm.txt")

SUBSTRATE_TO_GROUP = {
    "2,3-dihydroxybenzoic acid": "non-amino acid",
...
def plot_pycharts(tree, classifier, out_folder, label):
    ...
    for node_index, feature_index in enumerate(tree.feature):
        if feature_index >= 0:
            ...
            substrate_group_counts_left = get_substrate_group_counts(
                values_per_class_left, classifier.classes_, substrate_groups
            )
            substrate_group_counts_right = get_substrate_group_counts(
                values_per_class_right, classifier.classes_, substrate_groups
            )
            ...
            plot_pychart(
                list(map(int, substrate_group_counts_left)),
                substrate_groups,
                COLOUR_DICT,
                file_name_left,
            )
            plot_pychart(
                list(map(int, substrate_group_counts_right)),
                substrate_groups,
                COLOUR_DICT,
                file_name_right,
            )
