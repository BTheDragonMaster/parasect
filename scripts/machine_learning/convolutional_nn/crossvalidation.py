...
from parasect.scripts.machine_learning.convolutional_nn.convolution_3d import (
    train_loop,
    eval_loop,
    SUBSTRATE_PATH,
    SubstrateMetrics,
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory containing crossvalidation domain lists",
    )
    parser.add_argument(
        "-i",
        type=str,
    ...
    parser.add_argument("-nfc", type=int, default=3, help="Number of hidden fully connected layers.")
    parser.add_argument(
        "-dropout",
        type=float,
        nargs="?",
...
def crossvalidation() -> None:
    ...
    assert (
        args.nconv == len(args.stride) == len(args.pool) == len(args.padding) == len(args.convout) == len(args.kernel)
    )
    ...
    for i, crossval_pair in enumerate(crossval_pairs):
        ...
        scheduler = lr_scheduler.ReduceLROnPlateau(
            optimizer=optimizer, mode="min", factor=0.5, patience=5, min_lr=0.00001
        )
        ...
        for epoch in range(num_epochs):
            ...
            print(
                f"Epoch: {epoch + 1}/{num_epochs}; " f"LR: {lr}; " f"Train loss: ",
                train_loss,
                f"Test loss: ",
                test_loss,
                f"Accuracy: ",
