...
from parasect.scripts.data_processing.structure_processing.make_voxel_dataset import (
    VoxelDataset,
)
import parasect.data.structure_data

SUBSTRATE_PATH = os.path.join(os.path.dirname(parasect.data.structure_data.__file__), "included_substrates.txt")
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory to saved voxel features directory",
    )
    ...
    parser.add_argument(
        "-n",
        type=str,
        default=SUBSTRATE_PATH,
        help="Path to file containing included substrates",
    )
...
def write_aa_metrics(out_folder, epoch, metrics, specificities):
    ...
    with open(out_file, "w") as out:
        ...
        for aa in specificities:
            ...
            out.write(
                f"{aa}\t{aa_metrics.tp_count}\t{aa_metrics.fp_count}\t{aa_metrics.tn_count}\t{aa_metrics.fn_count}\n"
            )
...
def write_metrics(
    ...
    with open(out_file, "w") as out:
        out.write(
            "epoch\ttrain_loss\ttest_loss\taccuracy\tlearning_rate\tprecision\trecall\tclass_precision\tclass_recall\ttrain_accuracy\n"
        )
        for i, epoch in enumerate(epochs):
            out.write(
                f"{epoch}\t{train_losses[i]}\t{test_losses[i]}\t{accuracies[i]}\t{learning_rates[i]}\t{precisions[i]}\t{recalls[i]}\t{class_precisions[i]}\t{class_recalls[i]}\t{train_accuracies[i]}\n"
            )
...
def train_loop(
    model: nn.Module, loader: DataLoader, optimizer: Optimizer, criterion: ty.Callable
...
def main() -> None:
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer=optimizer, mode="min", factor=0.5, patience=5, min_lr=0.00001)
    ...
    for epoch in range(num_epochs):
        ...
        write_aa_metrics(
            aa_metrics_folder,
            epoch + start_epoch + 1,
            aa_metrics[epoch + start_epoch + 1],
            train.all_specificities,
        )

        print(
            f"Epoch: {epoch + 1 + start_epoch}/{num_epochs + start_epoch}; " f"LR: {lr}; " f"Train loss: ",
            train_loss,
            f"Test loss: ",
            test_loss,
            f"Accuracy: ",
