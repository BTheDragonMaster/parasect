...
from parasect.scripts.machine_learning.random_forest.multi_class_no_moments import (
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to input tabular containing constant-length specificity-conferring codes",
    )
...
class RandomForestParasect:
    ...
    def train_model(
        ...
        if sampling_method == "balanced":
            ...
            sequence_classifier = RandomForestClassifier(
            ...
            structure_classifier = RandomForestClassifier(
                n_estimators=n_estimators,
                n_jobs=n_jobs,
                oob_score=oob_score,
                random_state=25051989,
            ...
            if classifier.endswith(".parasect"):
                ...
                for i, true_y in enumerate(test_y):
                    ...
                    if true_y not in substrate_to_rates:
                        substrate_to_rates[true_y] = OrderedDict({"TP": 0, "FP": 0, "FN": 0})
                    if best_substrate not in substrate_to_rates:
                        substrate_to_rates[best_substrate] = OrderedDict({"TP": 0, "FP": 0, "FN": 0})
