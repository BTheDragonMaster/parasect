...
from parasect.scripts.data_analysis.plotting.plot_confusion_matrices import (
    plot_matrix,
    write_matrix,
)
...
class Dataset:
    def __init__(
        ...
        domain_to_substrate,
        domain_mappings,
        domain_mapping_categories,
        compound_mappings=None,
        compound_mapping_categories=None,
        mode="single_label",
    ):
            ...
            if domain in train_domain_list or domain in test_domain_list:
                ...
                if compound_mappings:
                    ...
                    if mode == "single_label":
                        ...
                        raise ValueError(
                            "Only 'single_label', 'multi_label' and 'binary' are accepted modes for domain-only input."
                        )
    ...
    def write_test_metrics(self, mode, predictions, out_dir, oob_score=None):
        ...
        if self.type == "single_label":
            ...
            for i, datapoint in enumerate(datapoints):
                ...
                domain_to_predictions[datapoint.domain].append(
                    (
                        prob_true,
                        datapoint.substrate,
                ...
                if true_y:
                    if not predicted_y:
                        ...
                        confusion_matrix[self.substrates.index(datapoint.substrate)][
                            self.substrates.index(datapoint.substrate)
                        ] += 1
                    ...
                    if not predicted_y:
                        ...
                        for true_substrate in datapoint.domain_substrates:
                            confusion_matrix[self.substrates.index(datapoint.substrate)][
                                self.substrates.index(true_substrate)
                            ] += 1
        ...
        with open(metrics_file, "w") as out:
            if self.type != "tandem":
                out.write(
                    "substrate\taccuracy\tnr_correct\tnr_incorrect\tprecision\trecall\tF1\tTP\tTP_probability\tFP\tFP_probability\tFN\tFN_probability\n"
                )
            else:
                out.write(
        ...
        if self.type == "single_label":
            ...
            boxplot = plt.boxplot(
                [tp_probabilities, fp_probabilities, fn_probabilities],
                vert=True,
                labels=["TP", "FP", "FN"],
                patch_artist=True,
            )
    ...
    def record_test_results(self, dataset, out_dir, mode="both"):
        ...
        if mode == "both" or mode == "test":
            ...
            if dataset.type == "tandem":
                ...
                plt.text(
                    recall[index] - 0.65,
                    precision[index] - 0.05,
                    f"Recommended threshold: {thresholds[index]:.2f}",
                    size=12,
                ...
                plt.text(
                    fpr[ix] + 0.05,
                    tpr[ix] - 0.05,
                    f"Recommended threshold: {thresholds[ix]:.2f}",
                    size=12,
                    fontdict=dict(size=10),
                    bbox=dict(facecolor="lightsteelblue", alpha=0.5),
                )
