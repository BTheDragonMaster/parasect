from parasect.scripts.parsers.parsers import (
    parse_moment_vectors,
    parse_bitvectors,
    parse_specificities,
    parse_domain_list,
)
from parasect.scripts.data_processing.get_domain_list import make_domain_mapping
...
class RFTrainingPoint:
    ...
    def build_vector(self, moment_mapping, bitvector_mapping):
        moment_vector = moment_mapping[self.domain]
        bitvector = bitvector_mapping[self.substrate]
        self.vector = moment_vector + bitvector


class RandomForestParasect:
    def __init__(self, bitvector_file, moment_file, specificities_file, domain_file):
    ...
    def test_model(self, test_domain_file, classifier_file):
        ...
        for i, true_y in enumerate(test_y):
            ...
            domain_to_predictions[tested_domain].append(
                (prob_true, tested_substrate, tested_substrate in true_substrates)
            )
        ...
        print(
            f"False positive: {FP} (average probability of interaction: {mean(probs_fp)}; median: {median(probs_fp)})"
        )
        print(f"True negative: {TN} (average probability of interaction: {mean(probs_tn)}; median: {median(probs_tn)})")
        print(
            f"False negative: {FN} (average probability of interaction: {mean(probs_fn)}; median: {median(probs_fn)})"
        )
        # pprint(fp_to_true)
        ...
        print(
            f"Correct percentage: {list(domain_to_correct.values()).count(True) / (list(domain_to_correct.values()).count(False) + list(domain_to_correct.values()).count(True))} "
        )
    ...
    def build_data(self, domain_file: str, under_sample=False, over_sample=False):
        ...
        for domain in domains:
            for substrate in self.substrate_to_bitvector:
                ...
                datapoint.build_vector(self.domain_to_moment_vector, self.substrate_to_bitvector)
                x.append(datapoint.vector)
                y.append(datapoint.interaction)
...
def run():
    ...
    random_forest = RandomForestParasect(args.b, args.m, args.s, args.d)
    if args.c:
        assert args.test
