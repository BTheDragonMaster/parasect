...
from parasect.scripts.feature_extraction.sequence_feature_extraction.read_positions import (
    POSITIONS_EXTENDED_SIGNATURE,
    POSITIONS_SIGNATURE,
)
from parasect.scripts.feature_extraction.sequence_feature_extraction.extract_domains import (
    parse_hmm3_results,
    parse_domain_id,
    parse_hmm2_results,
)
from parasect.scripts.feature_extraction.sequence_feature_extraction.hmm.run_hmmscan import (
    run_hmmscan,
    run_hmmpfam2,
)
import parasect.data.sequence_data.hmm
import parasect.data.sequence_data.sequences

REF_SEQ_FILE = os.path.join(
    os.path.dirname(parasect.data.sequence_data.sequences.__file__),
    "reference_sequence.fasta",
)
HMM_FILE_SEQUENCE = os.path.join(os.path.dirname(parasect.data.sequence_data.hmm.__file__), "AMP-binding_full.hmm")
HMM_FILE_HYBRID = os.path.join(os.path.dirname(parasect.data.sequence_data.hmm.__file__), "AMP-binding_hybrid.hmm")
HMM_FILE_STRUCTURE = os.path.join(
    os.path.dirname(parasect.data.sequence_data.hmm.__file__),
    "AMP-binding_structure.hmm",
)
HMM_FILE_NRPSPREDICTOR = os.path.join(
    os.path.dirname(parasect.data.sequence_data.hmm.__file__),
    "AMP-binding_nrpspredictor.hmm",
)
HMM_FILE_HMMER2 = os.path.join(os.path.dirname(parasect.data.sequence_data.hmm.__file__), "AMP-binding_hmmer2.hmm")
...
if __name__ == "__main__":
    ...
    print(
        "\t".join(
            map(
                str,
                find_hmm_positions(nrpspredictor_hit_to_id, POSITIONS_EXTENDED_SIGNATURE),
            )
        )
    )
