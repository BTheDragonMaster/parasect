...
def inventorise_sequences(sequence_files, specificity_files, file_labels, out_file):
    ...
    with open(out_file, "w") as out:
        out.write(
            "domain_id\tmibig_seq\tmibig_specificity\tparas_seq\tparas_specificity\tsandpuma_seq\tsandpuma_specificity\tnrpspredictor_seq\tnrpspredictor_specificity\n"
        )
        for domain_id, domain in id_to_domain.items():
            out.write(
                f"{domain_id}\t{domain.mibig_seq}\t{domain.mibig_spec}\t{domain.paras_seq}\t{domain.paras_spec}\t{domain.sandpuma_seq}\t{domain.sandpuma_spec}\t{domain.nrpspredictor_seq}\t{domain.nrpspredictor_spec}\n"
            )
