from parasect.scripts.parsers.tabular import Tabular
from parasect.scripts.parsers.parsers import (
    yield_identities,
    parse_specificities,
    parse_domain_list,
)
from parasect.scripts.parsers.fasta import read_fasta
...
def deduplicate_sequences(sequence_file, out_file):
    ...
    for domain_id in sequence_data.data:
        ...
        domain.add_specificity(
            "nrpspredictor",
            sequence_data.get_value(domain_id, "nrpspredictor_specificity"),
        )
    ...
    for domain_1 in domains:
        for domain_2 in domains:
            if domain_1.seq_id != domain_2.seq_id:
                if (
                    (domain_1.sequence[30:] in domain_2.sequence and domain_2.sequence[:100] in domain_1.sequence)
                    or (domain_1.sequence[:100] in domain_2.sequence and domain_2.sequence[30:] in domain_1.sequence)
                    or (domain_1.sequence in domain_2.sequence)
                    or (domain_2.sequence in domain_1.sequence)
                    or (domain_1.sequence == domain_2.sequence)
                ):
    ...
    with open(out_file, "w") as out:
        ...
        for domain in domain_groups:
            out.write(
                f"{domain.seq_id}\t{domain.sequence}\t{'|'.join(domain.mibig_spec)}\t{'|'.join(domain.paras_spec)}\t{'|'.join(domain.sandpuma_spec)}\t{'|'.join(domain.nrpspredictor_spec)}\n"
            )
    ...
    for domain_1 in domain_group:
        for domain_2 in domain_group:
            ...
            if domain_1 != domain_2 and pair not in assessed_pairs:
                if (
                    domain_1.mibig_spec
                    and domain_2.mibig_spec
                    and not domain_1.mibig_spec.intersection(domain_2.mibig_spec)
                ):
                ...
                if (
                    domain_1.paras_spec
                    and domain_2.paras_spec
                    and not domain_1.paras_spec.intersection(domain_2.paras_spec)
                ):
                    ...
                    has_conflict = True
                if (
                    domain_1.sandpuma_spec
                    and domain_2.sandpuma_spec
                    and not domain_1.sandpuma_spec.intersection(domain_2.sandpuma_spec)
                ):
                    ...
                    has_conflict = True
                if (
                    domain_1.nrpspredictor_spec
                    and domain_2.nrpspredictor_spec
                    and not domain_1.nrpspredictor_spec.intersection(domain_2.nrpspredictor_spec)
                ):
        ...
        for i, seq_1 in enumerate(sequences):
            for j, seq_2 in enumerate(sequences):
                if i != j and seq_1 and seq_2 and tuple(sorted([i, j])) not in evaluated_pairs:
                    if (
                        (seq_1[30:] in seq_2 and seq_2[:100] in seq_1)
                        or (seq_1[:100] in seq_2 and seq_2[30:] in seq_1)
                        or (seq_1 in seq_2)
                        or (seq_2 in seq_1)
                        or (seq_1 == seq_2)
                    ):
