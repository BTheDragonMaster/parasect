...
from parasect.scripts.parsers.voxel import voxels_from_file, Voxel
from parasect.scripts.math.shapes import Vector3D, Cube
from parasect.scripts.parsers.parsers import (
    parse_domain_list,
    parse_specificities,
    parse_substrate_list,
)
from parasect.scripts.data_processing.relabel_data import relabel_from_substrate_list
from parasect.scripts.data_analysis.count_substrates import count_substrates_from_file
        ...
        for i, plane in enumerate(grid):
            for j, row in enumerate(plane):
                for k, voxel in enumerate(row):
                    out.write(f"{i}\t{j}\t{k}\t")
                    attributes = voxel.vector.as_list(
                        categories=[
                            "nr_aromatic",
                            "nr_hydrophobic",
                            "nr_pos_charge",
