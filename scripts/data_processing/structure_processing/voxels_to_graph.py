...
from parasect.scripts.parsers.voxel import Voxel, voxels_from_file
...
def voxels_to_edges(voxels):
    ...
    sorted_voxels = sorted(
        ...
        key=lambda vox: (vox.cube.midpoint.x, vox.cube.midpoint.y, vox.cube.midpoint.z),
    )
...
def voxels_to_graph(voxels) -> Tuple[torch.tensor, torch.tensor, torch.tensor]:
    ...
    for voxel in sorted_voxels:
        node_features.append(
            [voxel.cube.midpoint.x, voxel.cube.midpoint.y, voxel.cube.midpoint.z] + list(astuple(voxel.vector))
        )
