...
from parasect.scripts.parsers.parsers import parse_specificities

import parasect.data

SPEC_FILE = os.path.join(os.path.dirname(parasect.data.__file__), "trustworthy_parasect_dataset.txt")
SPECIFICITIES = parse_specificities(SPEC_FILE)


def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Create fasta file containing all domains that recognise a certain substrate."
    )
