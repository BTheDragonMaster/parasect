...
from parasect.scripts.parsers.external_parsers import parse_sandpuma_data
from parasect.scripts.parsers.parsers import (
    parse_domain_list,
    parse_test_results,
    parse_specificities,
)

import parasect.data.structure_data
import parasect.data.train_test_splits
import parasect.data.benchmarking
import parasect.data

benchmarking_dir = os.path.dirname(parasect.data.benchmarking.__file__)


SUBSTRATE_FILE = os.path.join(os.path.dirname(parasect.data.structure_data.__file__), "included_substrates.txt")
...
DOMAINS_CLASS = os.path.join(os.path.dirname(parasect.data.train_test_splits.__file__), "class/test_filtered.txt")
DOMAINS_PHYLOGENY = os.path.join(
    os.path.dirname(parasect.data.train_test_splits.__file__),
    "phylogeny/test_filtered.txt",
)
...
PARASECT_DATASET = os.path.join(os.path.dirname(parasect.data.__file__), "parasect_dataset.txt")


def update_data(results_dir, method, domain_to_method_to_prediction, prefix="phylogeny"):

    structure_results = os.path.join(results_dir, f"{prefix}_structure/test_performance/test_results.txt")
    sequence_results = os.path.join(results_dir, f"{prefix}_sequence/test_performance/test_results.txt")
    sequence_structure_results = os.path.join(
        results_dir, f"{prefix}_sequence_structure/test_performance/test_results.txt"
    )

    files = [structure_results, sequence_results, sequence_structure_results]
...
def add_sandpuma_data(domain_to_method_to_prediction):
    ...
    for domain in domain_to_method_to_prediction:
        ...
        domain_to_method_to_prediction[domain]["SANDPUMA (prediCAT monophyly)"] = "|".join(
            domain_to_prediction[domain].predicat_mp
        )
        domain_to_method_to_prediction[domain]["SANDPUMA (prediCAT SNN)"] = "|".join(
            domain_to_prediction[domain].predicat_snn
        )
