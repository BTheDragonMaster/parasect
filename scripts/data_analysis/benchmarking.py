...
def plot_histogram(summary_file, out_plot, sequence_only=True):
    ...
    for datapoint in summary.data:
        ...
        for predictor in summary.categories[1:-1]:
            ...
            if predictor not in predictor_to_metrics:
                predictor_to_metrics[predictor] = {
                    "correct": 0,
                    "incorrect": 0,
                    "no call": 0,
                }
    ...
    for i, predictor in enumerate(predictors):
        ...
        correct, incorrect, no_call = (
            predictor_to_metrics[predictor]["correct"],
        ...
        print(
            ...
            correct / (correct + incorrect + no_call),
        )
...
def benchmark(results_dir, prefix="PARAS"):
    structure_results_phylogeny = os.path.join(results_dir, "phylogeny_structure/test_performance/test_results.txt")
    sequence_results_phylogeny = os.path.join(results_dir, "phylogeny_sequence/test_performance/test_results.txt")
    sequence_structure_results_phylogeny = os.path.join(
        results_dir, "phylogeny_sequence_structure/test_performance/test_results.txt"
    )
    structure_results_class = os.path.join(results_dir, "class_structure/test_performance/test_results.txt")
    sequence_results_class = os.path.join(results_dir, "class_sequence/test_performance/test_results.txt")
    sequence_structure_results_class = os.path.join(
        results_dir, "class_sequence_structure/test_performance/test_results.txt"
    )

    files = [
        structure_results_phylogeny,
