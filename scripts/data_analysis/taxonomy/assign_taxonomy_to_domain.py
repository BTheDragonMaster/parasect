...
from parasect.scripts.parsers.parsers import parse_taxonomy
import parasect.data.sequence_data.sequences
import parasect.data

DOMAINS = os.path.join(os.path.dirname(parasect.data.sequence_data.sequences.__file__), "domains.fasta")
TAXONOMY = os.path.join(os.path.dirname(parasect.data.__file__), "taxonomy.txt")
...
def assign_taxonomy(fasta_file, taxonomy_file, out_file):
    ...
    for seq_id_str in id_to_seq.keys():
        ...
        for protein in protein_list:
            if (
                protein in protein_to_taxonomy
                and protein_to_taxonomy[protein] not in taxonomy_list
                and protein_to_taxonomy[protein]
            ):
