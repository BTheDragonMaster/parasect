...
from parasect.core.parsing import parse_amino_acid_properties_file, parse_morgan_fingerprint_file, read_fasta_file
...
PROPERTIES_FILE: str = os.path.join(
    os.path.dirname(parasect.data.__file__), "physicochemical_properties.txt"
)
PROPERTIES: dict[str, list[float]] = parse_amino_acid_properties_file(PROPERTIES_FILE)
...
APOSITION_FILE_34_HMM2 = os.path.join(
    os.path.dirname(parasect.data.__file__), "active_site_hmm2.txt"
)
...
def align_adomain(domain_name, domain_sequence, alignment_file, temp_dir):
    ...
    id_to_alignment = read_fasta_file(temp_out)
...
class AdenylationDomain:
    ...
    def set_domain_signatures_hmm(self, hit_n_terminal, hit_c_terminal=None):
        ...
        signature = get_reference_positions_hmm(
            query, profile, [p - offset for p in signature_positions]
        )
        ...
        if hit_c_terminal:
            ...
            lysine = get_reference_positions_hmm(
                query_c, profile_c, [p - offset_c for p in position_k]
            )
        ...
        extended_signature = get_reference_positions_hmm(
            query, profile, [p - offset for p in extended_signature_positions]
        )
    ...
    def set_domain_signatures_profile(self, temp_dir):
        ...
        aligned_domain, aligned_reference = align_adomain(
            seq_id, self.sequence, ALIGNMENT_FILE, temp_dir
        )

        aligned_positions_signature = get_reference_positions(
            POSITIONS_SIGNATURE, aligned_reference
        )
        aligned_positions_extended_signature = get_reference_positions(
            POSITIONS_EXTENDED_SIGNATURE, aligned_reference
        )
    ...
    def write_sequence(self, fasta_file, sequence_type="full"):
        ...
        if sequence_type == "full":
            ...
            raise ValueError(
                f"Only accepted sequence types are 'full', 'signature', and 'extended_signature'. Got {sequence_type}"
            )
