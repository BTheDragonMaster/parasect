...
def _get_reference_positions_hmm(
    ...
    """Extracts the given positions from a query alignment.
    ...
    if len(reference_sequence) < len(reference_positions):
        raise ValueError(f"reference sequence is too short: {len(reference_sequence)} < {len(reference_positions)}")  # noqa: E501
    ...
    if not (
        ...
        raise ValueError(f"reference sequence is too long: {len(reference_sequence)} != {len(query_sequence)}")
...
def _align_adenylation_domain(
    ...
    run_muscle(
        path_in=temp_in, 
        path_in_alignment=alignment_file, 
        path_out=temp_out
    )
    aligned_domains = read_fasta_file(temp_out)
...
class AdenylationDomain:
    ...
    def set_domain_signatures_hmm(
        self, 
        hit_n_terminal: HSP,
        hit_c_terminal: Optional[HSP] = None
    ) -> None:
