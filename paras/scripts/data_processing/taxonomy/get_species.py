...
def retrieve_species(session: Session, out_dir: str, e_mail: str) -> None:
    """
    ...
    with open(out_tax, 'w') as out:
        out.write("protein_name\tspecies\tstrain\n")
        ...
        for synonym_batch in batches:
            ...
            for seq_record in SeqIO.parse(handle, "gb"):
                ...
                strain = seq_record.features[0].qualifiers.get("strain", [""])[0]
                out.write(f"{seq_record.id}\t{organism}\t{strain}\n")
                print(seq_record.id, organism, strain)
        handle.close()
...
def main():
    ...
    with Session(engine) as session:
        retrieve_species(session, args.out, args.email)
