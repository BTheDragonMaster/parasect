...
const DomainTile = ({result, domainIndex, protein_name, onAnnotationChange}) => {
    ...
    const recalculateAnnotationType = (substrates) => {
        const matchedNames = new Set((matchedSubstrates || []).map(s => s.name));
        ...
        const hasDropdownSelections = selectedDropdownNames.size > 0;
        const hasCustomSmiles = substrates.some(sub => sub.useCustomSmiles);
        ...
        // --- Branch 1: Duplicate domain with no sequence match
        if (!hasSequenceMatch && isDuplicateDomain) {
            type = 'duplicate_entry';

            // --- Branch 2: All dropdown-based (no custom SMILES)
        } else if (hasDropdownSelections && !hasCustomSmiles) {
            ...
            // --- Branch 3: All custom SMILES (no dropdown)
        } else if (!hasDropdownSelections && hasCustomSmiles) {
            type = hasSequenceMatch ? 'correction' : 'new_entry';

            // --- Branch 4: Mixed dropdown + custom SMILES
        } else if (hasDropdownSelections && hasCustomSmiles) {
            // Decide your intended behavior for mixed mode
            type = hasSequenceMatch ? 'correction' : 'new_entry';
        }
                ...
                fetch('/api/check_substrate_name', {
                    ...
                    .then((data) => {
                        ...
                        setSubstrates((prev) => {
                            ...
                            updated[i] = item;
                            recalculateAnnotationType(updated)
                            return updated;
                        });
                    })
                    .catch((err) => {
